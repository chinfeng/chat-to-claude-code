...
describe("SSE stream forwarding", () => {
  ...
  //         (HTTP 200)". The message embeds the `'"type":"overloaded_error"'`
  //         substring, which triggers a claude-code mid-stream client retry (a
  //         single-upstream proxy's substitute for cc-switch's pre-commit
  //         failover) — the accepted deviation from cc-switch's non-retry trade-off.
  // -----------------------------------------------------------------------
  ...
  it("surfaces an upstream connection abort as a stream_error SSE event that triggers a client retry", async () => {
    ...
    // emit the error event explicitly. We keep `error.type = "stream_error"`
    // (true root cause = connection abort, not a real 529) but embed the
    // `'"type":"overloaded_error"'` substring INTO `error.message`. The SDK
    // builds `e.message` from `error.message` (not `error.type`), and
    // claude-code's mid-stream retry predicate `sym` fires on that literal
    // substring inside `e.message` (HTTP 429/5xx is impossible mid-stream — 200
    // is already committed), so claude-code DOES retry this turn. A connection
    // reset/EOF is occassional network jitter worth re-running; this is our
    // single-upstream substitute for cc-switch's pre-commit provider failover,
    // which a single-upstream proxy cannot do.
    expect(eventTypes).toContain("error");
    const errorEvents = events.filter((e) => e.event === "error");
    expect(errorEvents.length).toBeGreaterThan(0);
    const parsed = JSON.parse(errorEvents[0].data);
    expect(parsed.error.type).toBe("stream_error");
    // error.type stays stream_error (true root cause = connection abort, not a
    // real 529). The `'"type":"overloaded_error"'` substring lives inside the
    // message — exactly what claude-code's `sym` substring match keys on, which
    // triggers a retry of the partially-streamed turn.
    ...
    // turn is persisted into conversation history (no poisoning). The client
    // DOES retry a partially-streamed turn on connection abort (intentional), but
    // that re-runs the turn with unchanged inputs — history stays clean.
  ...
  //         (→ event:error, stream_error, triggers client retry); [DONE] = no
  ...
  it("finalizes dump directory when upstream errors mid-stream", async () => {
    ...
    try {
      ...
      // `event: error` (stream_error; its message embeds the overloaded_error
      // substring to trigger a cc-switch-divergent client retry), and
      // finalizeDump must still run.
      const body = await collectResponseBody(res);
      expect(body).toContain("Upstream stream read error");
      expect(body).toContain('"type":"stream_error"');
      // The overloaded_error substring lives INSIDE the JSON-escaped `error.message`
      // field (shown as `\"type\":\"overloaded_error\"` in the raw data line) — parse
      // the error event so we assert the literal retry-trigger substring, matching
      // what claude-code's `sym` substring match sees.
      const errLine = parseSseResponse(body).find((e) => e.event === "error")!;
      expect(JSON.parse(errLine.data).error.message).toContain('"type":"overloaded_error"');
